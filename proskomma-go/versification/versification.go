...
type ForwardMappings struct {
	Mappings VrsMappings `js:"mappedVerses"`
}

type ReverseMappings struct {
	Mappings VrsMappings `js:"reverseMappedVerses"`
}
...
type MapVerseResult struct {
	BookCode      string
	VerseMappings []ChapterVerse
}
...
func bookCodeIndex() (map[string]int, map[int]string) {
	...
	bookCodes := [...]string{
		...
		"2PE", "1JN", "2JN", "3JN", "JUD", "REV", "TOB", "JDT", "ESG", "WIS", "SIR", "BAR",
		"LJE", "S3Y", "SUS", "BEL", "1MA", "2MA", "3MA", "4MA", "1ES", "2ES", "MAN", "PS2",
		"ODA", "PSS", "JSA", "JDB", "TBS", "SST", "DNT", "BLT", "EZA", "5EZ", "6EZ", "DAG",
...
func NewMapVerseResult() MapVerseResult {
	var m MapVerseResult
	m.VerseMappings = make([]ChapterVerse, 0)
	return m
}

// equivalent to vrs2json in proskomma-utils
func ForwardVersification(s string) ForwardMappings {
...
func succinctifyVerseMapping(v []VerseMappings, bci map[string]int, ibc map[int]string) (succinct.ByteArray, error) {
	...
	for _, vm := range v {
		...
		lb := recordLength + (recordType * 64)
		err := ba.SetByte(pos, uint8(lb))
		if err != nil {
			return ba, err
		}
	}
...
func MapVerse(s succinct.ByteArray, b string, c int, v int) (MapVerseResult, error) {
	m := NewMapVerseResult()
	m.BookCode = b
	pos := 0
	_, indexToBookCode := bookCodeIndex()
	...
	for pos < s.Length() {
		...
		recordPos += s.NByteLength(int(fromVerseStart))
		fromVerseEnd, err := s.NByte(recordPos)
		if err != nil {
			return m, err
		}
		recordPos += s.NByteLength(int(fromVerseEnd))
		if v < int(fromVerseStart) || v > int(fromVerseEnd) {
			pos += int(recordLenth)
			continue
		}
		bookCode := b
		if recordType == bcvMappingType {
			...
			bookCode = indexToBookCode[int(bookIndex)]
			recordPos += s.NByteLength(int(bookIndex))
		}
		m.BookCode = bookCode
		...
		recordPos += s.NByteLength(int(nMappings))
		for len(m.VerseMappings) < int(nMappings) {
			ch, err := s.NByte(recordPos)
			if err != nil {
				return m, err
			}
			recordPos += s.NByteLength(int(ch))
			verseStart, err := s.NByte(recordPos)
			if err != nil {
				return m, err
			}
			recordPos += s.NByteLength(int(verseStart))
			cv := ChapterVerse{
				Chapter: int(ch),
				Verse:   (v - int(fromVerseStart) + int(verseStart)),
			}
			m.VerseMappings = append(m.VerseMappings, cv)
		}
		break
	}

	if len(m.VerseMappings) == 0 {
		cv := ChapterVerse{
			Chapter: c,
			Verse:   v,
		}
