...
func TestConstructor(t *testing.T) {
	ba := NewByteArray(32)
	if cap(ba.ByteArray) != 32 {
		t.Errorf("bytes capacity for new ByteArray is %d not 32", len(ba.ByteArray))
	}
}
...
func TestDeleteItem(t *testing.T) {
	...
	if fullLength != (len(ba.ByteArray)) {
		t.Errorf("ByteArray expected to be length %d but was %d", fullLength, len(ba.ByteArray))
	}
	...
	if newFullLength != len(ba.ByteArray) {
		t.Errorf("newFullLength expected to be %d but was %d", len(ba.ByteArray), newFullLength)
	}
}
...
func TestInsert(t *testing.T) {
	...
	if int(tokenLength+scopeLength) != len(ba.ByteArray) {
		t.Errorf("sum of tokenLength and scopeLength expected to be %d but was %d", len(ba.ByteArray), int(tokenLength+scopeLength))
	}
	...
	if int(secondLength) != len(iba.ByteArray) {
		t.Errorf("secondLength expected to be %d but was %d", len(iba.ByteArray), int(secondLength))
	}
	...
	if int(firstLength+secondLength+thirdLength) != len(ba.ByteArray) {
		t.Errorf("sum of first/second/third lengths expected to be %d but was %d", len(ba.ByteArray), int(firstLength+secondLength+thirdLength))
	}
	...
	if int(fourthLength) != len(iba2.ByteArray) {
		t.Errorf("fourthLength expected to be %d but was %d", int(fourthLength), len(iba2.ByteArray))
	}

	if int(firstLength+secondLength+thirdLength+fourthLength) != len(ba.ByteArray) {
		t.Errorf("sum of first/second/third/fourth lengths expected to be %d but was %d", len(ba.ByteArray), int(firstLength+secondLength+thirdLength+fourthLength))
	}
}
